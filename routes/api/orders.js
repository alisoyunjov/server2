...
const upload = multer({
  ...
  router.post('/upload', upload.array('fileCollection', 5), async (req, res) => {
        ...
        const {milestone, numberOfFiles, numberOfItems, isFileAttached, orderStatus, daysSinceRequest, receivedDate, orderMadeBy, price1, price2, submission,requestDay, vendor1, vendor2, category, catalog1, catalog2, date, description,notes, msdFile} = req.body;
  ...
  router.put('/addFile/:id', upload.array('fileCollection', 5), async (req, res) => {
    ...
    Order.findOne({ _id: req.params.id }, (err, order) => {
        ...
        if (body.numberOfItems > 1){
          order.numberOfItems = body.numberOfItems;
        }
